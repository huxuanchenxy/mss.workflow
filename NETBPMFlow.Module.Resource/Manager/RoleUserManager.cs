...
namespace NETBPMFlow.Module.Resource.Manager
{
    ...
    /// 角色用户管理类
    /// </summary>
    internal class RoleUserManager:BaseRepo
    {
                /// <summary>
        /// 根据角色编码查询用户
        /// </summary>
        ...
        internal List<RoleUserView> GetUserByRoleIDs(string[] idsin)
        {
            ...
            {
                string sql = $@" SELECT a.id ID,a.id RoleID,CAST(a.id as CHAR(50)) RoleCode,a.role_name AS RoleName 
                                            ,b.id UserID,b.user_name UserName
                                            FROM role a INNER JOIN user b on a.id = b.role_id
                                            WHERE a.id IN @id ORDER BY a.id,b.id ";
                var dy = new DynamicParameters();
                dy.Add("id", idsin.ToArray());
                var data =  c.Query<RoleUserView>(sql,dy).ToList();
                return data;
            });
        }
        ...
        private List<User> GetUserListByRoles(string[] roleIDs, IDbSession session)
        {
            ...
            parm = parm.TrimEnd(',');
            string sql = $@" SELECT DISTINCT ID,user_name AS UserName FROM user WHERE role_id IN ("+ parm + ") ";
            //var userDbSet = session.GetRepository<UserEntity>().GetDbSet();
            //var roleUserDbSet = session.GetRepository<RoleUserEntity>().GetDbSet();
            //var roleDbSet = session.GetRepository<RoleEntity>().GetDbSet();
            //var list = (from u in userDbSet
            ...
            //            .Distinct()
            //            .ToList();
            var data = session.GetRepository<UserEntity>().Query(sql).ToList();
            var list = new List<User>();
            foreach (var d in data)
            {
                list.Add(new User() { UserID = d.ID.ToString(), UserName = d.UserName });
            }
            return list;
        }
        ...
        {
            string sql = $@" SELECT DISTINCT ID,user_name AS UserName FROM user WHERE role_id = '" + roleID + "' ";
            //var userDbSet = session.GetRepository<UserEntity>().GetDbSet();
            //var roleUserDbSet = session.GetRepository<RoleUserEntity>().GetDbSet();
            //var roleDbSet = session.GetRepository<RoleEntity>().GetDbSet();
            //var list = (from u in userDbSet
